...
def method_os_walk(path):
    """
    os.walk();會讀取整個資料夾內所有子資料夾與其文件
...
def method_os_listdir(path):
    """
    os.listdir();返回指定路徑下的文件和資料夾列表
...
def method_glob(path_pattern):
    """
    glob.glob();返回結果是匹配給定模式的帶完整路徑的檔名
...
def main():
    os_walk_path = ".."
    os_listdir_path = "../python-if__name__explain"
    glob_path_pattern = "../*/*"

    print("Method 1: Using os.walk")
    method_os_walk(os_walk_path)
    print("\nMethod 2: Using os.listdir")
    method_os_listdir(os_listdir_path)
    print("\nMethod 3: Using glob.glob")
    method_glob(glob_path_pattern)
